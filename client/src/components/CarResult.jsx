...
export default function CarResult({ make, model, year, confidence, notes, specs, imageUrl, loadingSpecs }) {
        ...
        {specs ? (
          <div className="mt-4 space-y-1">
            ...
            <SpecSection title="Power">
              <SpecRow
                label="Max power"
                value={ps ? `${ps} ps (${Math.round(ps * 0.9863)} hp)${specs.model_engine_power_rpm ? ` @ ${specs.model_engine_power_rpm} rpm` : ''}` : null}
              />
              <SpecRow
                label="Max torque"
                value={nm ? `${nm} Nm (${Math.round(nm * 0.7376)} lb-ft)${specs.model_engine_torque_rpm ? ` @ ${specs.model_engine_torque_rpm} rpm` : ''}` : null}
              />
            </SpecSection>
              ...
              <div className="mt-4">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    specs.modifications[key]?.length > 0 && (
                      <div key={key} className={`rounded-xl border p-3 ${color}`}>
                        ...
                        <div className="space-y-2">
                          {specs.modifications[key].map((mod, i) => (
          ...
          <p className="text-slate-500 text-sm mt-4 italic">Detailed specs not available</p>
        )}
      </div>
    </div>
  );
